...
Vocês se lembram do algoritmo de ordenação Insertion Sort?
Ele funcionava muito bem para os casos em que a lista estava crescente ou quase, como no exemplo abaixo:

INSERIR SLIDES DE EXEMPLO

Mas para os outros casos ele não era o mais eficiente, mas também não era a pior escolha.
Observe o exemplo abaixo. O que atrapalha essa situação?

INSERIR SLIDES DE EXEMPLO (quando o shell vai ser melhor que ele)

EXPLICAR COMO O SHELL É MELHOR AQUI

O shell sort trata-se de um refinamento do método de inserção, mencionado anteriormente. 
